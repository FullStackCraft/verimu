...
export class GitLabOrchestrator {
   ...
   */
  async scanInstance(config: GitLabScanConfig): Promise<GitLabScanResult> {
    ...
    if (config.groups && config.groups.length > 0) {
      // Fetch from specific groups
      const grouped: GitLabProject[] = [];
    ...
    // 3. Clone → Discover → Scan → Cleanup each repo
    const scannedRepos: GitLabRepoScanResult[] = [];
    const failedRepos: Array<{ project: GitLabProject; error: string }> = [];
  ...
  private filterProjects(
    ...
    toScan: GitLabProject[];
    skipped: Array<{ project: GitLabProject; reason: string }>;
  } {
    const toScan: GitLabProject[] = [];
    const skipped: Array<{ project: GitLabProject; reason: string }> = [];
    ...
    for (const project of projects) {
      ...
      if (config.excludePatterns?.length) {
        const matched = config.excludePatterns.some((pattern) =>
          this.matchGlob(project.path_with_namespace, pattern)
        );
        if (matched) {
  ...
  private aggregate(
    ...
    skippedRepos: Array<{ project: GitLabProject; reason: string }>,
    failedRepos: Array<{ project: GitLabProject; error: string }>,
    durationMs: number
  ): GitLabScanResult {
  ...
  private printSummary(result: GitLabScanResult): void {
    console.log('\n' + '═'.repeat(60));
